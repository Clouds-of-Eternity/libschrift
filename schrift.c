...
int sft_covered_in_table(const SFT *sft, uint32_t coverageStart, SFT_Glyph glyph)
{
	uint16_t coverageFormat = getu16(sft->font, coverageStart);
	uint16_t glyphCount = getu16(sft->font, coverageStart + 2);

	uint32_t min = 0;
	uint32_t max = glyphCount;
	...
	if (coverageFormat == 1)
	{
		while (min < max)
		{
			...
			uint32_t mid_index = getu16(sft->font, coverageStart + 4 + mid * 2);

			if (glyph > mid_index)
				min = mid + 1;
			else if (glyph < mid_index)
				max = mid;
			else
				return mid;
		}
	...
	else if (coverageFormat == 2)
	{
		...
		{
			uint32_t mid = min + (max - min) / 2;
			uint32_t startGlyphID = getu16(sft->font, coverageStart + 4 + mid * 6);
			uint32_t endGlyphID = getu16(sft->font, coverageStart + 4 + mid * 6 + 2);

			if (glyph > endGlyphID)
				min = mid + 1;
			else if (glyph < startGlyphID)
				max = mid;
			else
			{
				uint32_t startCoverageIndex = getu16(sft->font, coverageStart + 4 + mid * 6 + 4);

				return startCoverageIndex + glyph - startGlyphID;
			}
		}
	}
	return -1;
}
int sft_gpos(const SFT *sft, SFT_Glyph leftGlyph, SFT_Glyph rightGlyph, SFT_Kerning *kerning)
{
	uint_fast32_t offset;
	if (gettable(sft->font, "GPOS", &offset) < 0)
		return 0;

	/* Read GPOS table header. */
	if (!is_safe_offset(sft->font, offset, 4))
		return -1;
	if (getu16(sft->font, offset + 0) != 1)
		return 0;
	if (getu16(sft->font, offset + 2) != 0)
		return 0;

	uint16_t lookupListOffset = getu16(sft->font, offset + 8);
	uint32_t lookupListStart = offset + lookupListOffset;
	uint16_t lookupCount = getu16(sft->font, lookupListStart);
	for (int32_t i = 0; i < lookupCount; ++i)
	{
		uint16_t lookupTableOffset = getu16(sft->font, lookupListStart + 2 + 2 * i);
		uint32_t lookupTableStart = lookupListStart + lookupTableOffset;

		uint16_t lookupType = getu16(sft->font, lookupTableStart);
		if (lookupType != 2)
		{
			continue;
		}
		uint16_t subTableCount = getu16(sft->font, lookupTableStart + 4);
		uint16_t subTableOffsets = lookupTableStart + 6;
		for (int32_t c = 0; c < subTableCount; c++)
		{
			uint16_t subtableOffset = getu16(sft->font, subTableOffsets + 2 * c);
			// stbtt_uint8 *table = lookupTable + subtableOffset;
			uint32_t tableStart = lookupTableStart + subtableOffset;
			uint16_t posFormat = getu16(sft->font, tableStart);
			uint16_t coverageOffset = getu16(sft->font, tableStart + 2);

			int coverageIndex = sft_covered_in_table(sft, tableStart + coverageOffset, leftGlyph);
			if (coverageIndex == -1)
				continue;
